...
export function WaitTimeCard({ item, onDetails, onReport }: WaitTimeCardProps) {
  const crowd = crowdPresentation[item.crowdLevel];
    ...
    <View style={styles.card}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.statusRow}>
          <Ionicons name="people" size={17} color={crowd.color} />
          <Text style={[styles.crowd, { color: crowd.color }]}>{crowd.label}</Text>
          <Text style={styles.wait}>{item.estimatedMinutes === 0 ? 'No wait' : `${item.estimatedMinutes} min`}</Text>
        </View>
        <View style={styles.metaRow}>
          <Text numberOfLines={1} style={styles.updated}>Updated {item.lastUpdated}</Text>
